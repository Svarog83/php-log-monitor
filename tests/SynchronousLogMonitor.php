...
/**
 * Synchronous version of LogMonitor for testing
 */
final class SynchronousLogMonitor
{
    private ?LogFile $currentLogFile = null;
    private int $lastPosition = 0;
    private ?PositionTracker $positionTracker = null;
    private bool $isRunning = false;
    private float $lastRun = 0.0;
    ...
    public function __construct(
        ...
        $this->debugLogger->config("SynchronousLogMonitor initialized for project: {$this->project->name}");
    ...
    {
        $this->debugLogger->start("Starting SynchronousLogMonitor for project: {$this->project->name}");
        ...
        $this->debugLogger->success("SynchronousLogMonitor started successfully for project: {$this->project->name}");
    }

    public function stop(): void
    {
        $this->debugLogger->stop("Stopping SynchronousLogMonitor for project: {$this->project->name}");
        
        $this->isRunning = false;
        
        $this->debugLogger->success("SynchronousLogMonitor stopped for project: {$this->project->name}");
    }
    ...
    public function runCycle(): void
    {
        ...
        if ($currentTime - $this->lastRun >= $this->scanInterval) {
    ...
    public function runFor(float $seconds): void
    {
        ...
        while ($this->isRunning && microtime(true) < $endTime) {
            $this->runCycle();
            usleep(100000); // Sleep for 0.1 seconds to avoid busy waiting
        }
    }

    private function monitorCallback(): void
    {
    ...
    private function monitorCurrentLogFile(): void
    {
        ...
        $this->debugLogger->size("Current file size: {$currentSize} bytes, last position: {$this->lastPosition} bytes");
